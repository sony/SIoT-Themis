...
data "aws_vpc" "themis2_vpc" {
  filter {
    name   = "tag:Name"
    values = ["${var.sys_name}-${var.env}-vpc-${var.region}"]
  }
}

# ALB用セキュリティグループ作成
resource "aws_security_group" "alb_sg" {
  name        = "${var.sys_name}-${var.env}-alb-sg"
...
# Egress Rule（4041）
resource "aws_vpc_security_group_egress_rule" "out_4041_from_elb" {
  security_group_id = aws_security_group.alb_sg.id
  cidr_ipv4         = data.aws_vpc.themis2_vpc.cidr_block
  from_port         = 4041
  to_port           = 4041
  ip_protocol       = "tcp"
  description       = "Allow outbound 4041 to VPC"
}
