resource "aws_s3_bucket" "alb_logging" {
  ...
  tags = {
    Name = "${var.sys_name}-${var.env}-s3-alb-accesslog"
  }
}
    ...
    Statement = [
      {
        ...
        Action   = "s3:PutObject"
        Resource = "arn:aws:s3:::${var.sys_name}-${var.env}-s3-alb-accesslog/*"
      }
    ]
  })
}
...
resource "aws_s3_bucket_lifecycle_configuration" "alb_logging" {
  bucket = aws_s3_bucket.alb_logging.id
  rule {
    id = "${var.sys_name}-${var.env}-s3-alb-accesslog-lifecycle-rule"
    expiration {
