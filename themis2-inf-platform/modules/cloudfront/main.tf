...
resource "aws_cloudfront_distribution" "cf_themis2" {
  ...
  comment         = "CloudFront distribution for ${var.env}-${var.domain_name}"

  aliases = var.aliase_domains

  origin {
    domain_name = data.aws_lb.alb.dns_name
    origin_id   = "${var.sys_name}-${var.env}-alb"
  ...
  default_cache_behavior {
    target_origin_id = "${var.sys_name}-${var.env}-alb"
...
resource "aws_s3_bucket" "cloudfront_logging" {
  bucket        = "${var.sys_name}-${var.env}-cf-log"
  force_destroy = true
}
...
resource "aws_s3_bucket_lifecycle_configuration" "cloudfront_logging" {
  ...
  rule {
    id = "${var.sys_name}-${var.env}-cf-log-lifecycle-rule"
